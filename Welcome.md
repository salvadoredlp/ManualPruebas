# MANUAL DE BIENVENIDA EQUIPO MICROINFORMÁTICA
---

## Índice 

1. [USUARIOS](#3--usuarios)
2. [ACCESO A INFRAESTRUCTURA](#2--acceso-a-infraestructura)
3. [URL DE INTERÉS](#3--url-de-interés)
4. [INCIDENCIAS DE USUARIO](#4--incidencias-de-usuario)
5. [ORGANIGRAMA](#5--organigrama)
6. [CATÁLOGO DE SERVICIOS DE SISTEMAS Y COMUNICACIONES](#6--catálogo-de-servicios-de-sistemas-y-comunicaciones)
7. [VENTANA DE SERVICIOS](#7--ventana-de-servicio)
8. [EQUIPOS DE INFRAESTRUCTURA](#8--ventana-de-servicio)
9. [SERVICIOS CRÍTICOS](#9--equipos-de-infraestructura)
10. [MAPA GENERAL](#10--mapa-general)

---

### 1.- USUARIOS

**Nominal:** Es el usuario por defecto para todas las operativas de: 

	- Login en la plataforma
	- Acceso a herramientas  

**Administrador:** Solo se usa para acceso especiales como:

	- Acceso a cyberak
	- Acceso a máquinas

**Formato de las Firma de correo:**  

> **Nombre y Apellidos**  
> S.G. de Infraestructura Tecnológica Sanitaria
> **Nombre del Area**  
> Ministerio de Sanidad  
> Despacho: xxxx  
> Pso del Prado, 18-20 – 28014 Madrid  
> ☎️ Teléfono fijo- movil  
> ✉️ napellido@externos.sanidad.gob.es  
  
### 2.- ACCESO A INFRAESTRUCTURA 

Independientemente del método de acceso, se precisa generar un token de único asociado al usuario nominal. Se generará con la herramienta proporcionada por el equipo de Comunicaciones.  
Con el token hay que inventarse un PIN que se usará cuando hay que hacer login.   

El login habitual son 3 campos:  
	- Usuario: (suele ser napellido, lo habrán indicado en un correo junto con la contraseña)
	- Contraseña: La que venga asociada al usuario
	2ª contraseña: (nº aleatorio de un solo uso del token) + pin

VPN Se recibe un correo de comunicaciones indicando la instalación del cliente de la plataforma de VPN y del cliente para generar token 
CITRIX   https://teletrabajo.msc.es/Citrix/AplicacionesWeb/ 
Acceder al escritorio Gunix

Se pueden habilitar más aplicaciones en el escritorio citrix si fuera necesario, pero lo más cómodo es trabajar a través de la VPN e instalarse las herramientas en local.


### 3.- URL DE INTERÉS
Enlaces
### 4.- INCIDENCIAS DE USUARIO
Enlaces
### 5.- ORGANIGRAMA
Enlaces
### 6.- CATÁLOGO DE SERVICIOS DE SISTEMAS Y COMUNICACIONES
Enlaces
### 7.- VENTANA DE SERVICIO
Enlaces
### 8.- EQUIPOS DE INFRAESTRUCTURA
Enlaces
### 9.- SERVICIOS CRÍTICOS
Enlaces
### 10.- MAPA GENERAL


